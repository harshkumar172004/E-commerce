...
const Media = ({ media, handleDelete, deleteType, selectedMedia, setSelectedMedia }) => {
    ...
    return (
        <div className='border border-gray-200 daek:border-gray-800 relative group rounded overflow-hidden '>
                ...
                <DropdownMenu>
                    <DropdownMenuTrigger>
                        <span className='w-7 h-7 flex items-center justify-center rounded-full bg-black/50 cursor-pointer'>
                            <BsThreeDotsVertical className='text-[#fff]' />
                        </span>
                    </DropdownMenuTrigger>
                    <DropdownMenuContent align="start">
                        {deleteType === 'SD' &&
                            <>
                                ...
                                <DropdownMenuItem className='cursor-pointer' onClick={()=> handleCopyLink(media.secure_url)}>
                                    <IoIosLink color='black'/> Copy Link
                                </DropdownMenuItem>
                            </>
                        }
                        ...
                        <DropdownMenuItem className='cursor-pointer' onClick={()=> handleDelete([media._id],deleteType)} >
                            <LuTrash color='red'/>
                            {deleteType === 'SD' ? 'Move Into Tresh' : 'Delete Premanently'}
                        </DropdownMenuItem>
