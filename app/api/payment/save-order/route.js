...
export async function POST(request) {
    try {
        ...
        const orderSchema = zodSchema.pick({
            name: true,
            email: true,
            phone: true,
            country: true,
        ...
        if (!validate.success) {
            return response(false, 400, 'Invalied or missing fields', { error: validate.error })
        }
        ...
        try {
            ...
            await sendMail('Order placed sucessfully.', validatedData.email, orderNotification(mailData))

        } catch (error) {
            console.log(error)
        }

        return response(true,200,'order place successfully')
