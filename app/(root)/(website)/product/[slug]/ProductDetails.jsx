...
import { WEBSITE_PRODUCT_DETAILS, WEBSITE_HOME, WEBSITE_SHOP, WEBSITE_CART } from '@/routes/WebsiteRoute'
...
<Image src={loadingSvg} height={80} width={80} alt='loading.svg'/>
            </div>                  
}
            <div className="my-10">
                <Breadcrumb>
            ...
            <div className="md:flex justify-between items-start lg:gap-10 gap-5 mb-20">
                <div className="md:w-1/2 xl:flex xl:justify-center xl:gap-5 md:sticky md:top-0">
                    <div className="xl:order-last xl:mb-0 mb-5 xl:w-[calc(100%-144px)] border rounded">
                        <Image
                ...
                <div className='md:w-1/2 md:mt-0 mt-5'>
                    ...
                    <div className='flex items-center gap-2 mb-3'>
                        ...
                        <span className='bg-red-500 rounded-xl px-3 py-1 text-white text-sm ms-5'>-{variant.discountPercentage}%</span>
                    </div>
                    ...
                    <div className='mt-5'>
                        <p className='mb-2'>
                            <span className='font-semibold'>Color: <span className='font-light'>{variant?.color}</span></span>
                        </p>
                        <div className="flex gap-5 ">
                            {colors.map(color => (
                                <Link onClick={()=> setisProductLoading(true)} key={color} href={`${WEBSITE_PRODUCT_DETAILS(product.slug)}?color=${color}&&size=${variant.size}`} className={`border py-1 px-3 rounded-lg cursor-pointer hover:bg-primary hover:text-white ${color === variant.color ? 'bg-primary text-white' : ''}`}>{color}</Link>
                            ))}
                        </div>
                    </div>
                    <div className='mt-5'>
                        <p className='mb-2'>
                            <span className='font-semibold'>Size: <span className='font-light'>{variant?.size}</span></span>
                        </p>
                        <div className="flex gap-5 ">
                            {sizes.map(size => (
                                <Link onClick={()=> setisProductLoading(true)} key={size} href={`${WEBSITE_PRODUCT_DETAILS(product.slug)}?color=${variant.color}&&size=${size}`} className={`border py-1 px-3 rounded-lg cursor-pointer hover:bg-primary hover:text-white ${size === variant.size ? 'bg-primary text-white' : ''}`}>{size}</Link>
                            ))}
                        </div>
                    </div>
